...
AVRO_DICT_FILE = os.path.abspath("avrodict.json")
...
def process_match(match, fixed_text, cur, cur_end):
    ...
    if scope == 'punctuation':
        ...
        if (not ((chk < 0 and match['type'] == 'prefix') or
                 (chk >= len(fixed_text) and match['type'] == 'suffix') or is_punctuation(fixed_text[chk]))
            ^ negative):
            replace = False
    ...
    # -- value of 1 AND 2
        ...
        if (not (((chk >= 0 and match['type'] == 'prefix') or
                  (chk < len(fixed_text) and match['type'] == 'suffix'))
                 and is_consonant(fixed_text[chk]))
            ^ negative):
            replace = False
    # -- Exacts
    elif scope == 'exact':
        ...
        # Validate exact find.
        if not is_exact(match['value'], fixed_text, exact_start,
                                 exact_end, negative):
            replace = False
